...
def add_figure_caption(fig, caption: str) -> None:
    """Add a readable interpretation note below a Matplotlib figure."""
    fig.text(
        0.5,
        0.015,
        caption,
...
def plot_model(monotone_gp, x_scaler, y_mean: float, y_scale: float, 
    ...
    """
    Plots reconstructed flux law. Call once modified GP has been fit and 
...
def plot_slices(monotone_gp, x_scaler, y_mean: float, y_scale: float, 
               X_train_raw: np.ndarray, y_train_raw: np.ndarray,
               learn_neg_flux: bool, n_plot: int = 200, n_T_slices: int = 6,
               T_slices: np.ndarray | None = None):
    """
    Plots reconstructed flux law as T-slices.

    Each curve shows reconstructed q(s, T) versus s at a fixed T.
    Call once modified GP has been fit and predictions have been made.
    
    """
    s_plot = np.linspace(X_train_raw[:, 0].min(), X_train_raw[:, 0].max(),  
                         n_plot)

    if T_slices is None:
        T_slices = np.linspace(X_train_raw[:, 1].min(), X_train_raw[:, 1].max(),
                               n_T_slices)
    else:
        T_slices = np.asarray(T_slices)

    # Build prediction grid as fixed-T slices
    # Shape before flattening: len(T_slices) curves, each with n_plot s-values
    S = np.tile(s_plot, len(T_slices))
    TT = np.repeat(T_slices, n_plot)

    X_plot_raw = np.column_stack([S, TT])

    # GP was trained on standardized inputs, so standardize the plot grid too
    X_plot = x_scaler.transform(X_plot_raw)

    # Predict posterior mean of f on the slice grid
    f_plot_std, _ = monotone_gp.predict_mean_and_ds(X_plot)

    # Robust conversion in case GP returns torch tensors
    if hasattr(f_plot_std, "detach"):
        f_plot_std = f_plot_std.detach().cpu().numpy()

    f_plot_std = np.asarray(f_plot_std).reshape(-1)
    ...
    # Reshape into T-slices: one row per fixed T
    Q = q_plot_raw.reshape(len(T_slices), n_plot)

    # 2D T-slice plot
    fig, ax = plt.subplots(figsize=(8, 6))

    cmap = plt.cm.viridis
    norm = Normalize(vmin=T_slices.min(), vmax=T_slices.max())
    ...
    # Show training data projected onto q vs s, colored by T
    sc = ax.scatter(
        X_train_raw[:, 0],
        q_train_obs,
        c=X_train_raw[:, 1],
    ...
    ax.set_xlabel("s = dT/dx")
    ax.set_ylabel("reconstructed q(s, T)")
    ax.set_title("Reconstructed constitutive flux law: T-slices")
    ax.legend()
...
def plot_tangent_diagnostic(
    positive_tangent_true: np.ndarray,
    positive_tangent_pred: np.ndarray,
):
    """Compare held-out true and predicted positive tangents."""
    fig, ax = plt.subplots(figsize=(6, 5))
    ax.scatter(positive_tangent_true, positive_tangent_pred, s=20, alpha=0.7)
    ...
    ax.plot([lo, hi], [lo, hi], linestyle="dotted", label="ideal prediction")
    ax.axhline(0.0, linestyle="--", color="black", alpha=0.7)
    ax.set_xlabel("true positive tangent  -dq/ds")
    ax.set_ylabel("predicted positive tangent  -dq/ds")
    ax.set_title("EP monotonicity performance diagnostic")
    ax.legend()
    ax.grid(True, alpha=0.25)
    plt.tight_layout()
...
def plot_all_results(
    ...
    """
    Create the complete post-fit diagnostic plot suite with interpretations.

    This is the single plotting entry point intended for use at the end of an
    experiment, after the GP has been fit and held-out predictions are ready.
    The returned dictionary makes individual figures available for saving or
    further customization.
    """
    figures = {}

    figures["flux_surface"] = plot_model(
        monotone_gp, x_scaler, y_mean, y_scale, X_train_raw, y_train_raw,
        learn_neg_flux, n_plot=60,
    )
    add_figure_caption(
        figures["flux_surface"][0],
        "Interpretation: the surface is the posterior mean flux q(s, T); the "
        "points are noisy training observations. Close alignment indicates "
        "that the fitted constitutive law explains the observed flux data.",
    )

    figures["temperature_slices"] = plot_slices(
        monotone_gp, x_scaler, y_mean, y_scale, X_train_raw, y_train_raw,
        learn_neg_flux,
    )
    add_figure_caption(
        figures["temperature_slices"][0],
        "Interpretation: each curve holds temperature fixed, making the "
        "dependence of q on gradient s easier to compare. Smooth, consistently "
        "ordered curves reveal how temperature modifies the flux response.",
    )

    figures["gradient_surface"] = plot_derivs(
        monotone_gp, x_scaler, y_scale, X_train_raw, a_train_true,
        learn_neg_flux,
    )
    monotonicity_text = (
        "For q = -k s, physical monotonicity corresponds to dq/ds <= 0."
        if learn_neg_flux else
        "The imposed convention expects dq/ds >= 0."
    )
    add_figure_caption(
        figures["gradient_surface"][0],
        "Interpretation: the surface is the GP-implied tangent dq/ds and the "
        f"points are exact training tangents. {monotonicity_text}",
    )

    figures["tangent_diagnostic"] = plot_tangent_diagnostic(
        positive_tangent_true, positive_tangent_pred,
    )
    violation_rate = np.mean(np.asarray(positive_tangent_pred) < 0)
    add_figure_caption(
        figures["tangent_diagnostic"][0],
        "Interpretation: points on the dotted diagonal are exact tangent "
        "predictions; points below the horizontal zero line violate the "
        f"monotonicity condition. Observed violation rate: {violation_rate:.2%}.",
    )
...
def plot_derivs(
    ...
    """
    Plots reconstructed dq/ds.

    smooth_sigma controls visual smoothing of the plotted derivative surface.
    Use smooth_sigma=0.0 to show the exact GP prediction with no display smoothing.
    """
    ...
    # GP was trained on standardized inputs.
    X_plot = x_scaler.transform(X_plot_raw)

    # Predict posterior mean of df/ds on the grid.
    _, dfds_plot_std = monotone_gp.predict_mean_and_ds(X_plot)
    dfds_plot_std = np.asarray(dfds_plot_std).reshape(-1)

    # Convert from standardized df/ds back to raw df/ds units.
    sigma_s = x_scaler.scale_[0]
    dfds_plot_raw = (y_scale / sigma_s) * dfds_plot_std

    # Convert from f back to q if the model learned f = -q.
    if learn_neg_flux:
        dqds_plot_raw = -dfds_plot_raw
    else:
        dqds_plot_raw = dfds_plot_raw

    dQds = dqds_plot_raw.reshape(S.shape)

    # Smooth only the displayed surface, not the underlying GP predictions.
    dQds_display = dQds.copy()

    if smooth_sigma is not None and smooth_sigma > 0:
        try:
            from scipy.ndimage import gaussian_filter

            dQds_display = gaussian_filter(
                dQds_display,
                sigma=smooth_sigma,
                mode="nearest",
            )
        except ImportError:
            print("scipy not found; plotting unsmoothed derivative surface.")

    dqds_train_obs = np.asarray(a_train_true).reshape(-1)
    ...
    ax.set_xlabel("s = dT/dx", labelpad=12)
    ax.set_ylabel("T", labelpad=12)
    ax.set_zlabel("reconstructed dq/ds", labelpad=12)
    ax.set_title("Reconstructed derivative dq/ds", pad=18)
    ...
    cbar.set_label("reconstructed dq/ds")
...
def main():
    ...
    # Single post-fit call creates and displays the complete diagnostic suite.
    plot_all_results(
        monotone_gp=monotone_gp,
        x_scaler=x_scaler,
        y_mean=y_mean,
        y_scale=y_scale,
        X_train_raw=X_train_raw,
        y_train_raw=y_train_raw,
        a_train_true=a_train_true,
        positive_tangent_true=positive_tangent_true,
        positive_tangent_pred=positive_tangent_pred,
