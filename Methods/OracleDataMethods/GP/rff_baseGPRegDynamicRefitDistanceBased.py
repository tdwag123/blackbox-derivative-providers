...
Currently, we store K(X,X) + noise I.
For feature-space RFF, we want to store A = I + Phi.T Phi / noise.
"""
...
class GPFluxST:
    ...
    @staticmethod # can define in class without passing in self
...
def main():
    ...
    model = GPFluxST(s_train, T_train, q_train, noise_std=0.02, n_restarts_optimizer=0, max_cache_size=8)
