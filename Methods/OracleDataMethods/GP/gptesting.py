...
def fit_unregularized_reference(s_train, T_train, q_train):
    return fit_model(
        s_train,
        T_train,
        q_train,
...
def run_regularization_sweep(s_train, T_train, q_train, learned_noise_variance):
    ...
    for fraction, strength in zip(regularization_fractions, strengths):
        ...
        models[float(strength)] = model
        fit_times[float(strength)] = fit_time
    return models, fit_times, strengths

# MODEL EVALUATION
def evaluate_model(name, model, fit_time, S, TT, regularization_strength, regularization_fraction):
    ...
    return{
        ...
        "regularization_fraction_of_noise": regularization_fraction,
